...
export default function AddFixedBillSheet({ open, onClose, bill, categories }) {
  ...
  async function onSubmit(e) {
    ...
    setBusy(true);
    try {
      const payload = { name: name.trim(), amount: cents, categoryId, dueDay: day };
    ...
    <Sheet open={open} onClose={onClose} title={bill ? 'Edit Fixed Bill' : 'Add Fixed Bill'}>
      <form onSubmit={onSubmit} className="space-y-3 pb-4">
        ...
        <div className="text-ink-faint text-xs">
          Applied automatically on the {dueDay || '1'} of each new month as{' '}
          <span className="tnum">{formatDollars(toCents(amount))}</span>.
        </div>
